...
export function LanguageAssociationManager({
  ...
  useEffect(() => {
    async function loadLanguages() {
      try {
        const { data, error } = await supabase.from("languages").select("id, name, slug, logo_path").order("name")
  ...
  const addAssociation = async () => {
    ...
    try {
      ...
      if (isPrimary && associations.some((a) => a.isPrimary)) {
        ...
        if (!confirm("Un autre langage est déjà défini comme principal. Voulez-vous remplacer ce langage principal?")) {
          setIsLoading(false)
          return
        }
      }
  ...
  const removeAssociation = async (languageId: number) => {
    ...
    try {
      ...
      const isRemovingPrimary = associations.find((lang) => lang.id === languageId)?.isPrimary || false
      ...
      if (removedLanguage) {
        ...
        if (isRemovingPrimary) {
          toast({
            title: "Information",
            description: "Vous avez supprimé le langage principal. Vous pouvez en définir un nouveau si nécessaire.",
          })
        }
      }
    } catch (error) {
          ...
          <div className="flex flex-wrap gap-2">
            {associations.map((lang) => (
              <div key={lang.id} className="flex items-center">
                <Badge variant={lang.isPrimary ? "success" : "outline"} className="flex items-center gap-1 py-1 px-3">
      ...
      <div className="border p-4 rounded-md space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center space-x-2">
            <Switch id="is-primary" checked={isPrimary} onCheckedChange={setIsPrimary} disabled={isLoading} />
        ...
        <p className="text-sm text-gray-500 mt-2">
          Note: Un outil peut être associé à plusieurs langages, mais un seul peut être défini comme principal.
        </p>
      </div>
    </div>
  )
