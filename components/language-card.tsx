...
export function LanguageCard({ language }: LanguageCardProps) {
  ...
  // Assurons-nous que la fonction getSlug() fonctionne correctement
  // et que le lien est bien construit

  // Modifions la fonction getSlug pour s'assurer qu'elle retourne toujours une valeur valide
  // Remplacer la fonction getSlug existante par celle-ci
  const getSlug = () => {
    if (language.slug && language.slug.trim() !== "") return language.slug
    // Si le slug est vide ou null, générer un slug à partir du nom
    return generateLanguageSlug(language.name)
  }

  // Ajoutons un log pour déboguer
  console.log(`Langage: ${language.name}, Slug: ${getSlug()}`)
  ...
  // Utiliser l'ID comme fallback si le slug pose problème
  return (
    <Link
      href={language.slug ? `/language/${language.slug}` : `/language/id/${language.id}`}
      className="block h-full border-4 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transition-all duration-300 hover:-translate-y-1"
    ...
    >
      <div className="p-4 pb-6 flex flex-col h-full">
        ...
        <p className="text-xs text-gray-500 mb-2">Slug: {getSlug()}</p>
        ...
        <div className="mt-auto">
          ...
          <div
            className="w-full h-6 bg-gray-200 border border-black"
            role="progressbar"
