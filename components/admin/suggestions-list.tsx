...
export default function SuggestionsList() {
  ...
  const handleApproveCorrection = async (id: number) => {
    ...
    setCorrections(
      corrections.map(correction =>
        correction.id === id ? { ...correction, status: 'approved' } : correction,
      ),
    );
  };
  ...
  const handleRejectCorrection = async (id: number) => {
    ...
    setCorrections(
      corrections.map(correction =>
        correction.id === id ? { ...correction, status: 'rejected' } : correction,
      ),
    );
  };
    ...
    <div className="container mx-auto py-6 px-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          ...
          className="border-4 border-black hover:bg-yellow-300"
        >
