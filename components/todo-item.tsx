...
export function TodoItem({ todo, categories, statuses, onDelete, onUpdate }: TodoItemProps) {
  ...
  const formattedDueDate = todo.due_date ? format(new Date(todo.due_date), "d MMMM yyyy", { locale: fr }) : null

  // Gérer le changement de statut de complétion
  const handleToggleCompletion = async () => {
    try {
      setIsLoading(true)
      const updatedTodo = await todosApi.toggleTodoCompletion(todo.id, !todo.is_completed)
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3">
            <button
              ...
              aria-label={todo.is_completed ? "Marquer comme non terminée" : "Marquer comme terminée"}
            >
              {todo.is_completed ? <CheckCircle className="h-5 w-5 text-green-500" /> : <Circle className="h-5 w-5" />}
            </button>
            <div>
              <h3 className={cn("font-medium text-lg", todo.is_completed && "line-through")}>{todo.title}</h3>
