...
export default async function LanguageByIdPage({ params }: LanguagePageProps) {
  const { id } = params
  const languageId = Number.parseInt(id, 10)

  if (isNaN(languageId)) {
    notFound()
  }

  console.log("ID reçu:", languageId)

  try {
    const language = await getLanguageById(languageId)
    console.log("Langage trouvé par ID:", language ? language.name : "Non trouvé")

    if (!language) {
      console.error(`Langage non trouvé pour l'ID: ${languageId}`)
      notFound()
    }

    // Si le langage a un slug, rediriger vers l'URL avec le slug
    if (language.slug) {
