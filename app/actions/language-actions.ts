...
export async function createLanguageAction(formData: FormData) {
  try {
    ...
    const language: Partial<Language> = {
      ...
      popularFrameworks:
        (formData.get('popularFrameworks') as string)?.split(',').map(s => s.trim()) || [],
...
export async function updateLanguageAction(id: string, formData: FormData) {
  try {
    ...
    if (formData.has('shortDescription'))
      language.shortDescription = formData.get('shortDescription') as string;
    ...
    if (formData.has('isOpenSource'))
      language.isOpenSource = formData.get('isOpenSource') === 'true';
    ...
    if (formData.has('currentVersion'))
      language.currentVersion = formData.get('currentVersion') as string;
    ...
    if (dbData.short_description !== undefined)
      updateData.short_description = dbData.short_description;
