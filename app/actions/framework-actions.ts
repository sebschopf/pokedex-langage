...
export async function createFrameworkAction(formData: FormData) {
  try {
    ...
    const framework: Partial<Library> = {
      ...
      popularity: formData.has('popularity')
        ? Number.parseInt(formData.get('popularity') as string)
        : null,
...
export async function updateFrameworkAction(id: number, formData: FormData) {
  try {
    ...
    if (formData.has('languageId'))
      framework.languageId = Number.parseInt(formData.get('languageId') as string);
    if (formData.has('description'))
      framework.description = (formData.get('description') as string) || null;
    if (formData.has('website'))
      framework.officialWebsite = (formData.get('website') as string) || null;
    if (formData.has('githubUrl'))
      framework.githubUrl = (formData.get('githubUrl') as string) || null;
    ...
    if (formData.has('isOpenSource'))
      framework.isOpenSource = formData.get('isOpenSource') === 'true';
    ...
    if (dbData.official_website !== undefined)
      updateData.official_website = dbData.official_website;
