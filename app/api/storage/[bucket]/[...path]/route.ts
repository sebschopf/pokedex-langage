 ...
 */
export async function GET(request: NextRequest, { params }: { params: { bucket: string; path: string[] } }) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: `Fichier non trouvé: ${filePath}`, details: error.message }, { status: 404 })
    }
