...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (error) {
      return NextResponse.json({ message: "Erreur lors de la mise à jour de la proposition" }, { status: 500 })
    }
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    const supabase = createServerClient()
    const { error } = await supabase.from("language_proposals").delete().eq("id", Number.parseInt(params.id))

    if (error) {
      return NextResponse.json({ message: "Erreur lors de la suppression de la proposition" }, { status: 500 })
    }
