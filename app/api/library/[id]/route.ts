...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!updatedLibrary) {
      return NextResponse.json({ error: "Erreur lors de la mise à jour de la bibliothèque" }, { status: 500 })
    }
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!success) {
      return NextResponse.json({ error: "Erreur lors de la suppression de la bibliothèque" }, { status: 500 })
    }
