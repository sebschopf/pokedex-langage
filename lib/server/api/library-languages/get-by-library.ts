...
export async function getLanguagesForLibrary(libraryId: number) {
  try {
    ...
    if (error) {
      console.error(
        `Erreur lors de la récupération des langages pour la bibliothèque ${libraryId}:`,
        error,
      );
      throw error;
    }
    ...
    console.error(
      `Exception lors de la récupération des langages pour la bibliothèque ${libraryId}:`,
      error,
    );
    return [];
  }
