...
export async function getCorrectionWithLanguageById(id: number): Promise<CorrectionWithLanguage | null> {
...
export async function updateCorrection(id: number, correction: Partial<Correction>): Promise<Correction> {
  const supabase = createServerClient()
  ...
  if (correction.correctionText !== undefined) updateData.correction_text = correction.correctionText
