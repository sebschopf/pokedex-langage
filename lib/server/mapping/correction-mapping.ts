import type { DbCorrection } from "@/types/database"
import type { Correction } from "@/types/models"
import { toNumber, toString } from "@/utils/validation/type-conversion"
...
export function correctionToDb(correction: Partial<Correction>): Partial<DbCorrection> {
  ...
  if (correction.id !== undefined) {
    dbCorrection.id = correction.id
  }

  if (correction.correctionText !== undefined) {
    dbCorrection.correction_text = correction.correctionText
  }

  if (correction.createdAt !== null) {
    dbCorrection.created_at = correction.createdAt
  }

  if (correction.field !== null) {
    dbCorrection.field = correction.field
  }

  if (correction.framework !== undefined) {
    dbCorrection.framework = correction.framework
  }

  if (correction.languageId !== undefined) {
    dbCorrection.language_id = correction.languageId
  }

  if (correction.status !== undefined) {
    dbCorrection.status = correction.status
  }

  if (correction.suggestion !== undefined) {
    dbCorrection.suggestion = correction.suggestion
  }

  if (correction.updatedAt !== undefined) {
    dbCorrection.updated_at = correction.updatedAt
  }

  if (correction.userId !== undefined) {
    dbCorrection.user_id = correction.userId
  }
